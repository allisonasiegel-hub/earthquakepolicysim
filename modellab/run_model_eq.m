...
for kk = 1:sims
        kk;kk
%data='data_for_model_BS08B'; data2 = split(data, '_');
%data='data_for_model_Arad11'; data2 = split(data, '_');
data='data_for_model_JER2'; data2 = split(data, '_');
out_file_name = strcat(data2{end}, {' noA'});
%file='D:\MATLAB\EQ\BS08\'; % load and define
%file='D:\MATLAB\EQ\Arad\'; % load and define
%file='D:\MATLAB\EQ\TVR\'; % load and define
file='D:\MATLAB\EQ\JER\'; % load and define
load(data);
%% 
...
comm_policy=0;
%min_sal = 5300; % min salary according to BTL in 2022
%min_sal = 4300; % min salary according to BTL in 2012
min_sal = 6400; % min salary according to BTL in 2026
sims=30;
steps=760;
shock=0;
shock_step=9000000;
...
recovery_factor=4.5;
displaced_shelter=0; % shelter policy of public turn to 99
agents_per_sqm=0.2;
%% model parameters
[~,~,models_par] = xlsread([file,'model parameters.csv']);
JobsPerM_comm = models_par{strcmp(models_par(:,1),'JobsPerM_comm'), 2};
acts=3;
wactsnum=3;
wact1=0.5;
wact2=0.5;
...
for g=1:length(g_sa)
    ...
    SA_JOBS(g,1)=sum(Work_places(:,2)==g_sa(g) & Work_places(:,7)==1)/sum(Work_places(:,2)==g_sa(g) & Work_places(:,7)<2);    
    SA_IDLE(g,1)=sum(Individuals_data(:,2)==g_sa(g) & (Individuals_data(:,12)==1))/sum(Individuals_data(:,2)==g_sa(g) & Individuals_data(:,12)>0);            
    SA_COMMUTE(g,1)=sum(Individuals_data(:,2)==g_sa(g) & (Individuals_data(:,17)==99))/sum(Individuals_data(:,2)==g_sa(g) & Individuals_data(:,12)~=0);   
...
for i=1:steps
    ...
    %Occupied_Jobs=sum(Work_places(:,7)==1 | Work_places(:,7)==99)/sum(Work_places(:,7)~=2); % occupied/total
    ...
    if i>30  % after 30 simulation steps
        ...
        if ~isempty(B_D)
            ...
            working99 = randsample(size(B_D,1), round(commute_outside*size(B_D,1)));
            occ(working99) = 99;
        ...
        if  comm_policy==0
            ...
            if size(wp1,1)>0 
                ...
                working99 = randsample(size(WP,1), round(commute_outside*size(WP,1)));
                occ(working99) = 99;
    ...
    %% change average_wage
%    alfa=0.3;%0.4 % Ash 0.3
%    beta=0.8;%0.6 % Ash 0.8
%    lamda=0.45;%0.25 % Ash 0.45
%    delta=0.75;%0.8 % Ash 0.75
    ...
    %Occupied_Jobs_1=sum(Work_places(:,7)==1)/sum(Work_places(:,7)<2); % occupied ratio in area, excluding 99
    ...
    for g=1:length(g_sa) % unique SA ID ; next step calculations
        ...
        SA_JOBS(g,i+1)=sum(Work_places(:,2)==g_sa(g) & Work_places(:,7)==1)/sum(Work_places(:,2)==g_sa(g) & Work_places(:,7)<2);
        SA_IDLE(g,i+1)=sum(Individuals_data(:,2)==g_sa(g) & (Individuals_data(:,12)==1))/sum(Individuals_data(:,2)==g_sa(g) & Individuals_data(:,12)>0);        
        SA_COMMUTE(g,i+1)=sum(Individuals_data(:,2)==g_sa(g) & (Individuals_data(:,17)==99))/sum(Individuals_data(:,2)==g_sa(g) & Individuals_data(:,12)~=0);          
...
clearvars -except Assets Assets_P Build_Data Build_Data_p HH_data HH_data_P...
            ...
            SA_FIRST SA_SECOND SA_THIRD SA_FOURTH SA_FIFTH SA_SIXTH SA_SEVENTH SA_EIGHTH...
            SA_NINTH SA_TENTH SA_AREA SA_COMMUTE SA_JOBS SA_IDLE
full_file_name = fullfile(['output5\',char(out_file_name),' ',num2str(kk)]);
save(full_file_name);
end