...
%% read data 
[Assets,~,Assets_p1]=xlsread([file,'assets.csv']);
[dealData,~,dealData_p1]=xlsread([file,'deals.csv']);

%% get the last deal data
Deal_data=sortrows(dealData,[2,-3]);

%% connect deal data to assets
[locA,locB] = ismember(Assets(:,1), Deal_data(:,2));
locA = locA & locB > 0;     % keep only valid matches
locB = locB(locA);          % remove zero indices
Assets(locA,5:10) = Deal_data(locB,:);
Assets_P = [Assets_p1(1,:), dealData_p1(1,:)];

% clearvars -except Assets_P Assets file floor_hight file

%% load buildings
[Build_Data,Build_Data_p,~]=xlsread([file,'buildings.csv']);
...
% clearvars -except assets_data Build_Data Build_Data_p assets_data_P floor_hight
...
for i=1:length(stat)
    ...
    for j=1:size(stat_Build_Data,1)
        if stat_Build_Data(j,3)==1
            ...
            if ~isempty(F) && F > 0 && length(sintetic_asset_price) >= F
                ...
                extra_price = nan;
                if extra > 30
                    if length(sintetic_asset_price) >= F + 1
                        extra_price = sintetic_asset_price(F + 1);
                    elseif ~isempty(sintetic_asset_price)
                        extra_price = nanmean(sintetic_asset_price);
                    end
                    building_ass = [building_ass; [stat(i), stat_Build_Data(j,1), ind, extra, extra_price]];
                    ind = ind + 1;
                end
                sintetic_asset_price(1:F) = [];
                sintetic_asset_size(1:F) = [];
                if extra > 30 && ~isempty(sintetic_asset_price)
...
% clearvars -except Build_Data Build_Data_p Assets Assets_P file 'sa_data.csv'

%% if stat cannot be for living
[sa,sa_P]=xlsread([file,'sas.csv']);
ind=sa(sa(:,46)>1,1);
