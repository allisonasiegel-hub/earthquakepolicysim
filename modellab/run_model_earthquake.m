...
% SHELTERING POLICY EXTENSIONS VARIANT (this file): reduced-scope fork for
% the sheltering/reconstruction-extensions project (see
% shelter_policy_extensions_handoff.md). Same base ABM as the
% elderly-focused thesis work, but NOT elderly-specific and NOT built on
% the thesis parameters - this file is aligned to the colleague-updated
% baseline versions of who_is_moving.m, find_new_house_same_stat.m,
% find_new_house_yeshuv.m, find_new_house_sa_score.m, pref_hh.m,
% SA_score_old.m, and migration_19.m (no wservice/eld_movef/svc_filter,
% no Asset_Avail, no HH_MOVE_TRACK - those thesis-only parameters and
% trackers don't exist in this project). Added on top of that baseline:
...
%     percentage/elderly-premium rewrite (HH_subsidy_pct.m, subsidy_pct,
%     w_subsidy_eld), which is not used here.
%   - The find_new_house_sa_score.m bug fix (filtering possible_assets to
%     the SAs that actually pass the preference threshold before calling
%     new_house) is re-applied on top of the colleague's current version
%     of that file.
