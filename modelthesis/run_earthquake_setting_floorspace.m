function saved_path = run_earthquake_setting_floorspace(wservice, wservice_old, eld_movef, svc_filter, scenario_tag, steps, lu_update_every, lu_warmup, sa_update_every, resSearchLen, new_jobs_rank_thresh, lost_jobs_rank_thresh, lu_change_rank_lower, lu_change_rank_upper, lu_jobs_per_meter, lu_potential_jobs_per_meter, init_dataset_name, commute_outside_rate, wage_alfa, wage_beta, wage_lamda, wage_delta, elderly_search_mode, shock_step, eld_movef_old, displaced_shelter, use_staggered_relocation, service_radius_m, relocation_w_slow, relocation_p_slow, relocation_p_fast)
% FORK of run_earthquake_setting.m, pointing at
% run_model_earthquake_floorspace.m instead of run_model_earthquake.m --
% the only substantive difference between the two main scripts is that
% the service ratio (stat_data col 5/6 and Build_Data col 19) is
% floorspace-weighted instead of a raw building count. See
% building_service_ratio_floorspace.m's header for why this is a fork
% rather than a parameter toggle (it changes wservice/elderly-search
% behavior, not just a reported metric). All arguments and defaults below
% are identical to run_earthquake_setting.m -- keep the two in sync
% manually if either's defaults change.
%
% Minimal-testing wrapper around run_model_earthquake_floorspace.m,
% mirroring run_sweep_setting.m's existing pattern for
% nextchangesfortracker.m: pre-set fast-testing values (single replicate,
% short run, infrequent land-use updates) plus the three behavioral
% parameters, then run the real simulation script UNMODIFIED via run().
% This avoids forking/duplicating the 1300+ line simulation file itself
% a second time just for this wrapper.
%
...
% BUG FIX: same fix as run_earthquake_setting.m -- lu_update_every/
% lu_warmup/sa_update_every previously defaulted to the original,
% untested values (3/4/30) instead of the actually-validated 1/30/1
% (daily land-use updates starting day 30, daily macro updates).
...
% use_staggered_relocation: see run_earthquake_setting.m's matching
% comment for full rationale.
if nargin<27 || isempty(use_staggered_relocation); use_staggered_relocation=1; end
% service_radius_m: building-service-ratio/-density neighbor-search
% radius (default 400 = original/control). See
% run_model_earthquake_floorspace.m's matching comment.
if nargin<28 || isempty(service_radius_m); service_radius_m=400; end
% relocation_w_slow/p_slow/p_fast: override the two-population Bernoulli
% mixture's parameters (default = Nepal-fit values, see
% run_model_earthquake_floorspace.m's matching comment). Setting p_slow
% and p_fast to the SAME value collapses the mixture to a single flat
% daily probability for every displaced household, regardless of
% relocation_w_slow's value (the fast/slow split becomes irrelevant when
% both populations draw against the same rate) -- e.g. for a simple flat
% 1%/day: pass relocation_p_slow=0.01, relocation_p_fast=0.01.
if nargin<29 || isempty(relocation_w_slow); relocation_w_slow=0.3947; end
if nargin<30 || isempty(relocation_p_slow); relocation_p_slow=0.01394; end
if nargin<31 || isempty(relocation_p_fast); relocation_p_fast=0.20199; end
...
tmp_tag_file = sprintf('__run_earthquake_setting_floorspace_tmp_%s.mat', run_uid);

% run_model_earthquake_floorspace.m ends with the same hardcoded
% "clearvars -except [...]" as the original that wipes every variable in
% this function's workspace not on ITS list (run() shares this function's
% workspace) -- scenario_tag isn't on that list, so it would be gone by
% the time we get here. Stash it on disk instead, which survives
% regardless of what clearvars does to the workspace. The tag file itself
% is PID-suffixed (and floorspace-prefixed, distinct from the original
% wrapper's tag file) so concurrent processes never share a file.
save(tmp_tag_file, 'scenario_tag');

run('run_model_earthquake_floorspace.m');

% tmp_tag_file itself doesn't survive the script's clearvars (only run_uid
% is on its keep-list) -- rebuild the same filename from run_uid, which
% does survive, rather than adding wrapper-only state to the script's
% keep-list.
tmp_tag_file = sprintf('__run_earthquake_setting_floorspace_tmp_%s.mat', run_uid);
