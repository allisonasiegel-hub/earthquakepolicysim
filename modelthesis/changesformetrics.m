    ...
    if i>shock_step && buildings_added<=total_hits && total_hits~=0
        ...
        if num_buildings>0
            ...
            if ~isempty(HH_destroyed)
                ...
                displaced_data = [
                   selected_HH(:,2), ...   % HH ID
                   selected_HH(:,1), ...   % old SA
                   selected_HH(:,9), ...   % old yeshuv
                   selected_HH(:,10), ...  % old building
                   selected_HH(:,11), ...  % old asset
                   selected_HH(:,6), ...   % HH income
                   selected_HH(:,7), ...   % asiron
                   selected_HH(:,5), ...   % elderly count
                   i*ones(n,1), ...        % timestep
