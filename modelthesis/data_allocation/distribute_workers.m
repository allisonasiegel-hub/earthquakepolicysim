...
for i=1:length(u)
    ...
    worker_Metro_zone(isnan(worker_Metro_zone))=0; % defensive fallback: comm31/comm34 should be pre-filled by start_HH_2018up.m; if a gap slips through, treat as 0 commuters rather than "all remaining"
    ...
    for j=1:2
        ...
        elseif ~isempty(B) && worker_Metro_zone(j)>0
            ...
            locA=ismember(Work_places(:,6),ID);
            Work_places(locA,7)= Work_places(locA,7)+1;
            Work_places(locA,8)=Individuals_data(F(1:num_assign),14);
 ...
 % attach is_old_old as the final trailing column now -- this is the
 % last stage that writes fixed-index columns to Individuals_data, so
 % it's the first safe point to merge it in. Keyed by ind_id (col 1) so
 % it survives labor_datasample.m's earlier row regroup/reorder.
 Individuals_data(:,18)=0;
 [~,locA]=ismember(Individuals_data(:,1),is_old_old_lookup(:,1));
 Individuals_data(locA>0,18)=is_old_old_lookup(locA(locA>0),2);
 Individuals_data_P=[Individuals_data_P,'is_old_old (70+)'];
