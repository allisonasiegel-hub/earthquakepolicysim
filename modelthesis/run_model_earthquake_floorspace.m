...
% Staggered relocation eligibility -- same mechanism and toggle as
% run_model_earthquake.m, see that file's matching comment for full
% rationale and calibration source (2015 Nepal earthquake IDP data).
if ~exist('use_staggered_relocation','var'); use_staggered_relocation=1; end
if ~exist('relocation_w_slow','var'); relocation_w_slow=0.3947; end
if ~exist('relocation_p_slow','var'); relocation_p_slow=0.01394; end
if ~exist('relocation_p_fast','var'); relocation_p_fast=0.20199; end
if ~exist('pending_relocation','var'); pending_relocation=zeros(0,2); end
...
%% near buildings
% RADIUS EXPERIMENT (service-radius-test branch, FLOORSPACE fork):
% service_radius_m controls the building-service-ratio/-density radius
% (default 400 = original/control, matching run_model_earthquake.m's
% control config; prior work on this branch also tested 300/350/500/600/
% 700 -- see analysis_scripts/reports/radius*/table.md). Settable via
% run_earthquake_setting_floorspace.m so it doesn't require hand-editing
% and reverting a hardcoded value. building_service_density() below reads
% the same variable, so the neighbor-search radius and the land-area
% normalization constant (pi*radius^2) always stay in sync automatically.
if ~exist('service_radius_m','var'); service_radius_m=400; end
Build_Distance_matrix_250=building_within_D(Build_Data,250);
Build_Distance_matrix_400=building_within_D(Build_Data,service_radius_m);
...
%% building size
% FORK (floorspace-weighted service ratio): moved ahead of the service-
% ratio calculation below, since building_service_ratio_floorspace needs
% col(25) already populated (the original ordering computed floorspace
% AFTER the count-based building_service_ratio, which never needed it).
FLOORSPACE = [];
...
% FORK: floorspace-weighted variant (building_service_ratio_floorspace.m)
% instead of the original building-count-based building_service_ratio.m
% -- see that function's header comment for why this is a fork rather
% than a parameter toggle.
[Build_Data,Build_Data_p]=building_service_ratio_floorspace(Build_Data,Build_Data_p,Build_Distance_matrix_400);
%% building service density (land-area, reporting-only) -- same metric as
% run_model_earthquake.m's, independent of which formula (count or
% floorspace-weighted ratio) drives elderly relocation behavior in this
% fork. See building_service_density.m's header for full rationale.
[Build_Data,Build_Data_p]=building_service_density(Build_Data,Build_Data_p,Build_Distance_matrix_400,service_radius_m);
...
% BUG FIX: stat_service.m's col(2) is building-COUNT-based (commercial+
% public)/residential. ass_price.m divides Build_Data(:,19) (building
% service ratio) against this col(2) to get each building's normalized
% value multiplier -- but in this floorspace fork, col(19) is FLOORSPACE-
% weighted (building_service_ratio_floorspace.m, called above at line
% ~303), so that division was mixing a floorspace-weighted numerator
% against a count-based denominator. Silently affected every asset's
% initial price/monthly-cost (Assets col 12/13) -> affordability
% filtering for the entire run, in every floorspace-driven run this
% session prior to this fix. Override col(2) here with a floorspace-
% weighted version of the SAME (commercial+public)/residential formula
% (usage 2-3 = commercial, usage 5 = public), matching col(19)'s units.
% stat_service.m itself is left untouched -- run_model_earthquake.m
% (count-based script) still gets the original count-based col(2).
g_sa_init_fix = unique(Build_Data(:,4));
for g0fix = 1:length(g_sa_init_fix)
    b0fix = Build_Data(Build_Data(:,4)==g_sa_init_fix(g0fix),:);
    compub_floor = sum(b0fix((b0fix(:,3)>1 & b0fix(:,3)<4) | b0fix(:,3)==5, 25));
    res_floor = sum(b0fix(b0fix(:,3)==1, 25));
    if res_floor==0
        stat_data(stat_data(:,1)==g_sa_init_fix(g0fix),2) = 0;
    else
        stat_data(stat_data(:,1)==g_sa_init_fix(g0fix),2) = compub_floor/res_floor;
    end
end
...
% FORK: floorspace-weighted (col-25 sums) instead of a raw building count,
% matching the dynamic col(5) update later in the loop.
g_sa_init = unique(Build_Data(:,4));
for g0 = 1:length(g_sa_init)
    b0 = Build_Data(Build_Data(:,4)==g_sa_init(g0),:);
    com0 = sum(b0(b0(:,3)>1 & b0(:,3)<4, 25)); % commercial floorspace (usage 2-3)
    res0 = sum(b0(b0(:,3)==1, 25)); % residential floorspace
    if res0==0
...
% col(7)/(8) = fixed land area (m^2) per SA and SA-level land-area
% service density -- same reporting-only metric as run_model_earthquake.m,
% independent of this fork's floorspace-weighted ratio. See that file's
...
%% start running
for i=1:steps
    % TEMP DIAGNOSTIC: periodic progress marker (floorspace fork only) --
    % added after a run hung with no error and near-zero CPU usage for
    % 17+ hours with no way to tell which day it stalled on. Remove once
    % the hang is understood/fixed.
    if mod(i,20)==0 || i==1
        fprintf('[progress] day %d/%d, %s\n', i, steps, datestr(now));
    end
    %toc
    % sum data
    average_wage=mean(Individuals_data(Individuals_data(:,15)>0 & Individuals_data(:,15)~=99,14));
    ...
    if i==shock_step && shock==0
        ...
        if ~isempty(HH_destroyed)
            ...
            % relocation) -- see the matching comment in
            % run_model_earthquake.m for full rationale.
    ...
    % -- see the matching comment in run_model_earthquake.m for full
    % rationale.
    newly_eligible_hh = [];
    if use_staggered_relocation==1
        if ~isempty(pending_relocation)
            % BUG FIX: see the matching comment in run_model_earthquake.m
            % -- prune pending households that no longer exist in HH_data
            % (deleted via the ordinary resSearchLen eviction path while
            % still waiting on their staggered-eligibility draw) before
            % drawing, or find_new_house_same_stat crashes on a nonexistent
            % household ID.
    ...
    %% sas move:
    % calculate building service ratio (FORK: floorspace-weighted)
    [Build_Data]=building_service_ratio_floorspace(Build_Data,Build_Data_p,Build_Distance_matrix_400);
    % calculate building service density (land-area, reporting-only)
    [Build_Data]=building_service_density(Build_Data,Build_Data_p,Build_Distance_matrix_400,service_radius_m);
    ...
    if mod(i,sa_update_every)==0
        ...
        for g=1:nSA % unique SA ID ; next step calculations
            ...
            % FORK: floorspace-weighted (sum of col-25 floorspace) instead
            % of a raw building count -- see building_service_ratio_floorspace.m's
            % header for why this is forked rather than a toggle.
            com_b=sum(b_data(b_data(:,3)>1 & b_data(:,3)<4, 25)); % commercial floorspace (usage 2-3)
            res_b=sum(b_data(b_data(:,3)==1, 25)); % residential floorspace (usage 1)
            if res_b==0
            ...
            % only, independent of this fork's floorspace-weighted ratio.
            la_g = stat_data(stat_data(:,1)==g_sa(g),7);
            if ~isempty(la_g) && ~isnan(la_g) && la_g>0
                stat_data(stat_data(:,1)==g_sa(g),8) = com_b/la_g; % com_b already computed as commercial floorspace above
            end
...
clearvars -except Assets Assets_P Build_Data Build_Data_p HH_data HH_data_P...
            ...
            pending_relocation relocation_w_slow relocation_p_slow relocation_p_fast use_staggered_relocation service_radius_m
