...
data='data_for_model_JER08'; data2 = split(data, '_');
out_file_name = strcat(data2{end}, {' no'});
%file='D:\MATLAB\EQ\BS08\'; % load and define
file='D:\MATLAB\EQ\jer2\'; % load and define
load(data);
%% 
...
for i=1:steps
    ...
    if i>30  % after 30 simulation steps
        ...
        if ~isempty(B_D)
            working99_prob = 0.530; % Ashkelon commuting 99 probability
        ...
        if  comm_policy==0
            ...
            JOBS_per_meter=0.51433061; % B7 0.013952158 ; Jer 0.51433061
    ...
    %% MODEL SHUK HAVODA:
    commute_outside=0.53; % Ash 0.379; B7 0.597; Jer 0.530
    ...
    %% change average_wage
    alfa=0.25;%0.4 % Ash 0.3
    beta=0.9;%0.6 % Ash 0.8
    lamda=0.9;%0.25 % Ash 0.45
    delta=0.55;%0.8 % Ash 0.75
...
full_file_name = fullfile(['EQ2\',char(out_file_name),' ',num2str(kk)]);
save(full_file_name);
end