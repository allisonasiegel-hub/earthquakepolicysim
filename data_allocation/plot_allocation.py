...
City-agnostic: pass --city to pick a city folder (JER, BS08, Arad, ASH22, ...)
under modellab, matching main_alloc.m's file/NAME convention. --mat/--census/
--out can still be overridden individually. The elderly-household choropleth
maps need a shapefile (only built so far for Tiberias, ../tveriashape/); if
--shape isn't given and the default path doesn't exist for the chosen city,
that panel is skipped rather than erroring.
...
def plot_elderly_maps(d, pdf, shape_path):
    try:
    ...
    if not os.path.exists(shape_path):
        print(f'[skip] no shapefile at {shape_path} -- skipping elderly maps')
        return
...
def main():
    ...
    ap.add_argument('--city', default='JER',
                     help="City folder name under modellab (JER, BS08, Arad, ASH22, ...) -- "
                          "matches main_alloc.m's file/NAME convention. Used to derive default "
                          "--mat/--census/--out paths when those aren't given explicitly.")
    ap.add_argument('--mat', default=None,
                     help='defaults to data_allocation/data_for_model_<city>.mat')
    ap.add_argument('--census', default=None,
                     help='defaults to <city>/sa_data_b7.csv')
    ap.add_argument('--out', default=None,
                     help='defaults to data_allocation/plots_allocation_<city>')
    ap.add_argument('--shape', default=SHAPE_PATH,
                     help='shapefile for elderly-household maps; skipped if missing')
    args = ap.parse_args()

    mat_path = args.mat or os.path.join(HERE, f'data_for_model_{args.city}.mat')
    census_path = args.census or os.path.join(HERE, '..', args.city, 'sa_data_b7.csv')
    out_dir = args.out or os.path.join(HERE, f'plots_allocation_{args.city}')
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        plot_elderly_maps(d, pdf, args.shape)
