...
eld_movef=0; %reduction in movement for elderly
...
service_mean = mean(stat_data(:,2));
service_std  = std(stat_data(:,2));
...
'Service_Elderly',...
'Service_NonElderly',...
'Service_Displaced_Elderly',...
'Service_Displaced_NonElderly',...
'HousingCost_Elderly',...
'HousingCost_NonElderly',...
'HousingCost_Displaced_Elderly',...
'HousingCost_Displaced_NonElderly',...
'ElderlyPop'
...
for i=1:steps
    ...
    if isempty(moving_HH)==0 % assign new asset for agent
        [HH_ID_left,HH_data,Assets,HH_change,LU,new_A,new_B,Build_Data,Asset_Avail]...
            ...
            Build_Data,Build_Distance_matrix_400,Assets,wresd,moving_HH,LU,new_A,new_B,HH_change,Asset_Avail);
    ...
    displaced = ismember(HH_data(:,2), ...
    HH_MOVE_TRACK(HH_MOVE_TRACK(:,5)==1,1));

    disp_elderly = elderly & displaced;
    disp_nonelderly = nonelderly & displaced;
    ...
    [~,locAsset] = ismember(asset_id,Assets(:,3));
   
    
    HH_cost = Assets(locAsset,13);
    ...
    Metric_Track(i,4)=mean(HH_service(disp_elderly),'omitnan');
    Metric_Track(i,5)=mean(HH_service(disp_nonelderly),'omitnan');
    
    % Housing costs
    
    Metric_Track(i,6)=mean(HH_cost(elderly),'omitnan');
    Metric_Track(i,7)=mean(HH_cost(nonelderly),'omitnan');
    Metric_Track(i,8)=mean(HH_cost(disp_elderly),'omitnan');
    Metric_Track(i,9)=mean(HH_cost(disp_nonelderly),'omitnan');
    Metric_Track(i,10)=sum(HH_data(:,5)>=2);

    % Available assets for moving HH (Asset_Avail: [HH_ID, isElderly, num_possible_assets])
    if size(Asset_Avail,1)>0
        Metric_Track(i,11)=mean(Asset_Avail(Asset_Avail(:,2)==1,3),'omitnan'); % avg available assets - elderly
        Metric_Track(i,12)=mean(Asset_Avail(Asset_Avail(:,2)==0,3),'omitnan'); % avg available assets - non-elderly

        Asset_Avail_displaced = ismember(Asset_Avail(:,1), ...
            HH_MOVE_TRACK(HH_MOVE_TRACK(:,5)==1,1));

        Metric_Track(i,13)=mean(Asset_Avail(Asset_Avail(:,2)==1 & Asset_Avail_displaced,3),'omitnan'); % avg available assets - displaced elderly
        Metric_Track(i,14)=mean(Asset_Avail(Asset_Avail(:,2)==0 & Asset_Avail_displaced,3),'omitnan'); % avg available assets - displaced non-elderly
    else
        Metric_Track(i,11)=NaN;
        Metric_Track(i,12)=NaN;
        Metric_Track(i,13)=NaN;
        Metric_Track(i,14)=NaN;
    end
...
Metric_Change = nan(1,8);

Metric_Change(1)=Metric_Track(end,2)-Metric_Track(1,2); % Elderly service
Metric_Change(2)=Metric_Track(end,3)-Metric_Track(1,3); % Non-elderly service
Metric_Change(3)=Metric_Track(end,4)-Metric_Track(1,4); % Displaced elderly service
Metric_Change(4)=Metric_Track(end,5)-Metric_Track(1,5); % Displaced non-elderly service

Metric_Change(5)=Metric_Track(end,6)-Metric_Track(1,6); % Elderly cost
Metric_Change(6)=Metric_Track(end,7)-Metric_Track(1,7); % Non-elderly cost
Metric_Change(7)=Metric_Track(end,8)-Metric_Track(1,8); % Displaced elderly cost
Metric_Change(8)=Metric_Track(end,9)-Metric_Track(1,9); % Displaced non-elderly cost
